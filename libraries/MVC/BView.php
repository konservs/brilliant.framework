...
class BView {
	...
	public function breadcrumbs_add_marka($id, $cat) {
		...
		$children = array();
		foreach ($brands as $b) {
			$children[$b->id] = (object)array('active' => true, 'url' => '//' . $brouter->generateURL('auto', BLang::$langcode, array('view' => 'brand', 'id' => $b->id)), 'name' => $b->getname());
		}
	...
	public function breadcrumbs_add_model($marka, $id) {
		...
		$children = array();
		foreach ($models as $m) {
			$children[$m->id] = (object)array('active' => true, 'url' => '//' . $brouter->generateURL('auto', BLang::$langcode, array('view' => 'model', 'id' => $m->id)), 'name' => $m->getname());
		}
	...
	public function breadcrumbs_add_catregion($catid, $id) {
		...
		$children = array();
		foreach ($regions as $r) {
			$children[$r->id] = (object)array('active' => true, 'url' => '//' . $brouter->generateURL('classified', BLang::$langcode, array('view' => 'category_region', 'category' => $catid, 'region' => $r->id)), 'name' => $r->getname());
		}
		$name = $reg->getname();
		return $this->breadcrumbs_add($url, $name, true, 'region', $children);
	}
	...
	public function breadcrumbs_add_catcity($catid, $id) {
		...
		if (!empty($subcities)) {
			$children = array();
			foreach ($subcities as $c) {
				$children[$c->id] = (object)array('active' => true, 'url' => '//' . $brouter->generateURL('classified', BLang::$langcode, array('view' => 'category_city', 'category' => $catid, 'city' => $c->id)), 'name' => $c->getname());
			}
			return $this->breadcrumbs_add($url, $name, true, 'city', $children);
		}
		return $this->breadcrumbs_add($url, $name, true, 'city');
