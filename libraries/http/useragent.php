<?php
//============================================================
// Sets of functions and classes to work with UserAgent
// Client type detection (mobile/desktop/tablet)
//
...
class BBrowserUseragent{
	protected static $devicetype=DEVICE_TYPE_UNKNOWN;
	protected static $devicesuffix='';
	//=====================================================
	...
	//=====================================================
	// List of mobile devices (phones) headers.
	//=====================================================
	...
	//
	//=====================================================
	protected static $httpHeaders=NULL;
	//=====================================================
	//
	//=====================================================
	...
	//=====================================================
	//  Set the HTTP Headers. Must be PHP-flavored. This
	// method will reset existing headers.
	//=====================================================
	public static function setHttpHeaders(){
		if(!is_array(self::$httpHeaders)){
	...
	public static function checkHttpHeadersForMobile(){
		foreach(self::getMobileHeaders() as $mobileHeader=>$matchType){
			if(isset(self::$httpHeaders[$mobileHeader])){
				if(is_array($matchType['matches']) ){
					foreach($matchType['matches'] as $_match)
						if(strpos(self::$httpHeaders[$mobileHeader],$_match)!==false)
							return true;
					return false;
					}else{
					return true;
	...
	public static function match($regex, $userAgent = null){
		...
		return (bool) preg_match('/'.$regex.'/is', (!empty($userAgent) ? $userAgent : NULL));
		}
	//=====================================================
	// Find a detection rule that matches the current
	// User-agent.
	//=====================================================
	public static function matchDetectionRulesAgainstUA(){
		$userAgent=isset(self::$httpHeaders['HTTP_USER_AGENT'])?self::$httpHeaders['HTTP_USER_AGENT']:'';
		if(empty($userAgent))
			return false;
	...
	//=====================================================
	// Returns TRUE if User Agent is mobile device
	//=====================================================
	...
	//=====================================================
	// Returns TRUE if User Agent is tablet
	//=====================================================
	...
	public static function detectDevice(){
		if($_COOKIE['v']=='m'){
			self::$devicetype=DEVICE_TYPE_MPHONE;
			}
		elseif($_COOKIE['v']=='d'){
			self::$devicetype=DEVICE_TYPE_OTHER;
			}
		if(self::$devicetype==DEVICE_TYPE_UNKNOWN){
			self::$devicetype=self::isMobile()?(self::isTablet()?DEVICE_TYPE_TABLET:DEVICE_TYPE_MPHONE):DEVICE_TYPE_OTHER;
			}
		if(empty($_COOKIE['v'])){
			if(self::$devicetype==DEVICE_TYPE_MPHONE||self::$devicetype==DEVICE_TYPE_TABLET){
				$v='m';
				}
			elseif(self::$devicetype==DEVICE_TYPE_OTHER){
				$v='d';
				}
			$expire=time()+2592000;//30 days
			setcookie('v',$v,$expire,'/',BHOSTNAME);
			}
	...
	//
	//=====================================================
	public static function getDeviceSuffix(){
		if(empty(self::$devicesuffix)){
